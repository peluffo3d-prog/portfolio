...
const AtlasModelViewer = dynamic(() => import("./AtlasModelViewer"), { ssr: false });

function ProjectCard({ project, index }: { project: Project; index: number }) {
  const cardRef = useRef<HTMLAnchorElement>(null);
  const cardInView = useInView(cardRef, { margin: "0px 600px 0px 600px" });
  const is3D = project.tags.includes("Three.js");
  ...
  return (
    ...
    >
      <div className="relative w-full h-full overflow-hidden" style={{ borderRadius: "4px" }}>
        {is3D ? (
          <div className="absolute inset-0" style={{ background: "radial-gradient(circle at 50% 50%, #1a1a1a 0%, #060606 75%)", pointerEvents: "none" }}>
            {cardInView && <AtlasModelViewer />}
          </div>
        ) : project.image ? (
          <Image
            src={project.image}
